# 将labelme标注的分割文件转化为cellpose可用的tif格式的标注，可用于训练cellpose
# 对于实例分割，每一类放置于一个文件夹内
import os
import numpy as np
import labelme
...
for anno in all_anno:
    ...
    for i in range(1, len(class_dir)):
        ...
        label_path = os.path.join(class_dir[i], image_name_without_ext + cellpose_anno_ext)
        # 保存tif格式的标注文件
        label = Image.fromarray(temp_ins)
        label.save(label_path)
        # 同一文件夹下保存原图
        shutil.copy(labelme_image_path, class_dir[i])